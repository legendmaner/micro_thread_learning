...
{
    MT_ATTR_API_SET(MONITOR_MT_POOL_SIZE, _total_num); // 微线程池大小
    ...
    MT_ATTR_API(MONITOR_MT_POLL_EMPTY, 1); // pool no nore
    if (_total_num >= _max_num)
    {
        MT_ATTR_API(MONITOR_MT_OVERLOAD, 1); // no more quota
        return NULL;
    }
    ...
    if ((NULL == thread) || (false == thread->Initial()))
    {
        MT_ATTR_API(MONITOR_MT_POLL_INIT_FAIL, 1); // pool init fail
...
#else
    if ((_last_clock - check_time) > 1000)
    {
        CNetMgr::Instance()->RecycleObjs(_last_clock);
        check_time = _last_clock;
    }
#endif
}
...
inline void MtFrame::InsertSleep(MicroThread* thread)
{
    ...
    if (rc < 0)
    {
        MT_ATTR_API(MONITOR_MT_HEAP_ERROR, 1); // heap error
